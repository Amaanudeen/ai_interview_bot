...
# Max follow-up questions allowed per main question
MAX_FOLLOWUPS_PER_QUESTION = 4
...
if not GEMINI_API_KEY:
    ...
    print("Please set it in the .env file: GEMINI_API_KEY='your-api-key'")
else:
...
class InterviewState(TypedDict):
    """LangGraph state for interview flow"""
    mode: str  # "role" or "resume"
...
class SpeechTranscriber:
    ...
    def transcribe_array(self, audio_data: np.ndarray, sample_rate: int) -> str:
        ...
        temp_file = None
        try:
            # Create temporary file with proper context manager
...
class TTSEngine:
    ...
    def speak(self, text: str) -> None:
        ...
        print(f"🔊 Speech queued: {len(text)} characters")
...
class LLMEvaluator:
    """
    Evaluates answers and generates questions using Gemini 2.0 Flash.
    """
    ...
    def generate_question(self, state: InterviewState) -> str:
        ...
        if mode == "role":
            prompt = f"""You are conducting a technical interview for the role: {role_or_resume}

This is question #{question_count + 1} in the interview.
{history_context}

Generate ONE relevant technical interview question for this role. The question should:
- Be specific and technical
- Test practical knowledge or problem-solving
...
Return ONLY the question text, nothing else."""
            ...
            prompt = f"""You are conducting a technical interview based on this resume/background:
{role_or_resume}

This is question #{question_count + 1} in the interview.
{history_context}

Generate ONE relevant technical interview question based on the candidate's resume. The question should:
- Be related to their experience or skills mentioned in the resume
- Test depth of knowledge in their stated expertise
- Be different from previous questions
- Be clear and concise
...
Return ONLY the question text, nothing else."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"Question generation failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    raise RuntimeError(f"Failed to generate question after {max_retries} attempts: {str(e)}")
    ...
    def evaluate_answer(self, question: str, answer: str, context: str = "") -> Dict[str, Any]:
        ...
        prompt = f"""You are evaluating a technical interview answer. Provide a detailed assessment.

Question: {question}

Answer: {answer}

Context: {context if context else "General technical interview"}
...
Be thorough but fair in your evaluation. Return ONLY valid JSON, no other text."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"JSON parsing failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    print(f"Failed to parse evaluation JSON after {max_retries} attempts")
                    return {
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"Evaluation failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    raise RuntimeError(f"Failed to evaluate answer after {max_retries} attempts: {str(e)}")
    ...
    def generate_followup(self, question: str, answer: str, evaluation: Dict[str, Any]) -> str:
        ...
        prompt = f"""You are conducting a technical interview. The candidate's answer needs clarification.

Original Question: {question}

Candidate's Answer: {answer}

Issues identified: {weaknesses_text}

Generate ONE follow-up question that:
- Addresses the gaps or weaknesses in their answer
- Helps them demonstrate deeper understanding
- Is specific and focused
- Is encouraging but probing
...
Return ONLY the follow-up question text, nothing else."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if followup:
                    ...
                    raise ValueError("Empty follow-up generated")
                    
            except Exception as e:
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"Follow-up generation failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    raise RuntimeError(f"Failed to generate follow-up after {max_retries} attempts: {str(e)}")
    ...
    def generate_consolidated_feedback(self, conversation_history: List[Dict[str, Any]]) -> str:
        ...
        prompt = f"""You are providing final feedback for a technical interview. Review the entire conversation and provide comprehensive feedback.

{conversation_summary}

Generate consolidated feedback that includes:
...
Make the feedback constructive, specific, and actionable. Be thorough but concise.
...
Return the complete feedback text."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"Feedback generation failed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
                    time.sleep(wait_time)
                else:
                    raise RuntimeError(f"Failed to generate consolidated feedback after {max_retries} attempts: {str(e)}")
